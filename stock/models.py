...
class PortfolioStock(DateTimeModel):
    stock = models.ForeignKey(Stock, on_delete=models.CASCADE,related_name="portfolio_stock",null=True,blank=True)
    quantity = models.IntegerField()
    buying_price = models.DecimalField(max_digits=10, decimal_places=2)
    buying_date = models.DateField(null=True,blank=True,auto_now= True)
    selling_date = models.DateField(null=True,blank=True)

    
    def __str__(self):
        return f"{self.stock.symbol} - {self.quantity} * {self.buying_price}"
